...
namespace nilnul.fs.file_.link_
{
	...
	static public class _SymX
	{
		...
		static public void Ensure(SpearI1 link, SpearI1 target)
		{
			if (nilnul.fs.address_.spear.be_.Newable.Singleton.be(link))
			{
				nilnul.fs.location_.link_.sym_.file.create_.ret_.success._ThrowIfFailX.Void(
		...
		public static void Create(SpearI1 link, SpearI1 target)
		{
			//Create(new ShieldDoc1(link), target);
			nilnul.fs.location_.link_.sym_.file.create_.ret_.success._ThrowIfFailX.Void(
				link.ToString()
		...
		{
			Create(en, kept.file.en);
		}

		public static void Create(SpearI1 en, FileI1 kept)
		{
			Create(en, kept.file.ee);
		}
