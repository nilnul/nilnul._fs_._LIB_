...
namespace nilnul.fs.folder_.nonlink.dirs_
{
	static public class _NonlinkX
	{
		static public IEnumerable<string> Captions(DirectoryInfo folder)
		{
			return folder.EnumerateDirectories().Where(
				x =>! nilnul.fs.folder.be_.link_._ThisSymX.Be(x)
			).Select(x => x.Name);
		}

		public static IEnumerable<string> Captions(FolderI folder)
		{
			return Captions(nilnul.fs.folder.to_._InfoX.To(folder) );
		}
		...
		{
			return parent.EnumerateDirectories().Where(
				x => ! nilnul.fs.folder.be_.link_._ThisSymX.Be(x)
			);
		}
