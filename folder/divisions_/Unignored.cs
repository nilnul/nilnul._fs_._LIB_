...
namespace nilnul.fs.folder.divisions_
{
	...
	static public class _Self0unignoredX
	{
		...
		static public IEnumerable<_address._route.DivisionI> Divisions(
			nilnul.fs.FolderI1 folder
			,
			_address._route.division_._div._dir.DntI ignoreDoc
		)
		{
					...
					case _address._route.division_._div.DirI dir:
						foreach (
							var division in fs.folder.divisions_._Self0unignoredX.Divisions(
								...
								,
								ignoreDoc
							).Select( dir_s_Division => _address._route.division_._DirDivisionX.Concat(dir, dir_s_Division) )
						)
						{
