...
namespace nilnul.fs.folder.destinations_
{
	...
	/// <summary>
	/// the documents and the directories that is not neglected:
	///		either according to cfg or to convention
	/// </summary>
	static public class _ConsideredX
	{
		...
		/// <returns></returns>
		static public IEnumerable<_address._route.division_. DestinationI> Destinations(
			nilnul.fs.FolderI1 folder
			,
			_address._route.division_._div._dir.DntI  ignoreDoc=null
		) {
			ignoreDoc = ignoreDoc ?? dnts_._ignored.SettingsX.Dnt;

			return nilnul.fs.folder.dsts_._ConsideredX.Dsts(folder,ignoreDoc).SelectMany(
				d => {
					switch (d)
					{
						...
						case _address._route.division_._div.DirI dir:
							return 
							  nilnul.fs.folder.divisions_._Self0consideredX.Divisions(
