...
namespace nilnul.fs.location.nulables.zip_
{
	public static class _ToFileX
	{
		...
		/// 
		/// </summary>
		/// <param name="contents"> the key ues denotes in order to deduplicate divisionRoute; the value maybe null</param>
			...
			)
			{
				...
				foreach (KeyValuePair<_address._dst.denote.str_.seq.Join, _location_.BlankI> item in contents)
				{
					switch (item.Value)
					{
						case null:
							archive.CreateEntry(
								new nilnul.fs._address._dst.denote.str.join.Capped(
									item.Key
								).ToString()
							);
							break;
