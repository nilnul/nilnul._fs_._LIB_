...
namespace nilnul.fs.location.nulables.zip_.toFile
{
	public static class _ToTmpX
	{
		...
		)
		{
			...
			return _ToFileX.File(contents, address);

		}
		public static nilnul.fs.FileI File(
			Dictionary<string, nilnul.fs._location_.BlankI> locations
			,
			nilnul.txt_._vered.Name zipMainName
		)
		{ 
		...
		public static nilnul.fs.FileI File(
			Dictionary<string, nilnul.fs._location_.BlankI> dict
			,
			string zipMainName
		)
		{
